...
@WebServlet(name = "UploadTeaList",urlPatterns = {"/UploadTeaList"})
@MultipartConfig
public class UploadTeaList extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (null == con) {
            ...
            List<Teacher> ls=ref.getExcelInfoTeacherlist(new File(savePath,newFileName));
            PreparedStatement ps=null;
            try{
                ps=con.prepareStatement("insert into teacher values (?,?,?,?,?,?)");
                if(ls!=null){
                    for(int i=0;i<ls.size();i++){
                        ps.setString(1,ls.get(i).getT_id());
                        ps.setString(2,ls.get(i).getT_name());
                        ps.setString(3,ls.get(i).getT_password());
                        ps.setString(4,ls.get(i).getT_sex());
                        ps.setString(5,ls.get(i).getT_college());
                        ps.setString(6,ls.get(i).getT_department());
                        ps.addBatch();
                    }
                    ps.executeBatch();
                }
